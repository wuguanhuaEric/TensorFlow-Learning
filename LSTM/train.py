...
def train(train_data, vocab_size, num_layers, num_epochs, batch_size,
          model_save_name, learning_rate = 1.0, max_lr_epoch = 10,
    ...
    with tf.Session() as sess:
        ...
        # 为了用 Saver 来保存模型的变量
        saver = tf.train.Saver(max_to_keep=10) # max_to_keep 默认是 5， 只保存最近的 5 个模型参数文件

        # 开始 Epoch 的训练
        for epoch in range(num_epochs):
            # 只有 Epoch 数大于 max_lr_epoch (设置为 10) 后，才会是学习率衰减
            ...
            for step in range(training_input.epoch_size):
                ...
                if step % print_iter != 0:
                    ...
                    # 每 print_iter (默认是50) 打印当下的 Cost (误差 / 损失) 和 Accuracy (精度)
                    print("Epoch {}, Step {}. Cost:{:.3f}, Accuracy:{:.3f}, Seconds per step:{:.3f}".format(epoch, step,
                            cost,acc, seconds) )
