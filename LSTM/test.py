...
def test(model_path, test_data, vocab_size, id_to_word):
    ...
    # hidden_size, num_steps, num_layers, vocab_size, batch_size等
    ...
    with tf.Session() as sess:
        ...
        # 第二维时 2 是因为对每一个 LSTM 单元有两个来自上一单元的输入
        ...
        for batch in range(num_acc_batches):
            if batch == check_batch_idx:
                true, pred, current_state, accuracy = sess.run([m.input_obj.targets, m.predict, m.state, m.accuracy],
